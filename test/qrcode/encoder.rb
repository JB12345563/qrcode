...
require "qrcode"
require_relative "../../fixtures/qrcode/sample_data"

describe QRCode do
	it "raises argument error for invalid inputs" do
		expect do
			QRCode::Code.new(size: 1, level: :h)
		...
		expect do
			assert QRCode::Code.new("10000000000000000000", size: 41, level: :h)
		end.to raise_exception(ArgumentError, message: be == "Given size greater than maximum possible size of 40")
		
		expect do
			assert QRCode::Code.new("hello", size: 1, level: :a)
	...
	it "creates a QR code with level H" do
		...
		qr = QRCode::Code.new("duncan", size: 1)
		expect(qr.modules).to be == MATRIX_1_H
		qr = QRCode::Code.new("duncan", size: 1, level: :l)
		expect(qr.modules).to be == MATRIX_1_L
		qr = QRCode::Code.new("duncan", size: 1, level: :m)
		expect(qr.modules).to be == MATRIX_1_M
		qr = QRCode::Code.new("duncan", size: 1, level: :q)
		expect(qr.modules).to be == MATRIX_1_Q
	end
	
	it "creates a QR code with level H and size 3" do
		qr = QRCode::Code.new("duncan", size: 3)
	...
	it "creates a QR code with level H and size 5" do
		qr = QRCode::Code.new("duncan", size: 5)
	...
	it "creates a QR code with level H and size 10" do
		qr = QRCode::Code.new("duncan", size: 10)
	...
	it "creates a QR code with level H and size 4" do
		qr = QRCode::Code.new("www.bbc.co.uk/programmes/b0090blw", level: :l, size: 4)
		expect(qr.modules).to be == MATRIX_4_L
		qr = QRCode::Code.new("www.bbc.co.uk/programmes/b0090blw", level: :m, size: 4)
		expect(qr.modules).to be == MATRIX_4_M
		qr = QRCode::Code.new("www.bbc.co.uk/programmes/b0090blw", level: :q, size: 4)
		expect(qr.modules).to be == MATRIX_4_Q
		
		qr = QRCode::Code.new("www.bbc.co.uk/programmes/b0090blw")
	...
	it "creates a QR code with level H and size 1" do
		...
		QRCode::Code.new("1234567890", size: 1, level: :h)
		
		qr = QRCode::Code.new("DUNCAN", size: 1, level: :h)
		expect(qr.to_s[0..21]).to be == "xxxxxxx xxx   xxxxxxx\n"
	end
	
	it "creates a QR code with level H and size 1" do
		expect(QRCode::Code.new("DUNCAN", mode: :alphanumeric).mode).to be == :mode_alpha_numk
		
		expect do
			QRCode::Code.new("Duncan", mode: :alphanumeric)
		end.to raise_exception(ArgumentError)
	end
	
	it "automatically uses numeric mode" do
		# When digit only automatically uses numeric mode, default ecc level is :h
		digits = QRCode::Code.new("1" * 17) # Version 1, numeric mode, ECC h
		...
		alpha = QRCode::Code.new("X" * 10) # Version 1, alpha mode, ECC h
		...
		binary = QRCode::Code.new("x" * 7) # Version 1, 8bit mode, ECC h
	...
	it "creates a QR code with level H and size 2" do
		...
		qr = QRCode::Code.new(data, size: 2, level: :m, mode: :number)
		expect(qr.to_s[0..25]).to be == "xxxxxxx   x x x   xxxxxxx\n"
	end
	
	it "does not throw an error for valid inputs" do
		QRCode::Code.new("2 1058 657682")
		QRCode::Code.new("40952", size: 1, level: :h)
		QRCode::Code.new("40932", size: 1, level: :h)
		QRCode::Code.new("", size: 1, level: :h)
		QRCode::Code.new("0", size: 1, level: :h)
	end
	
	it "raises an error for exceeding max size" do
		expect do
			QRCode::Code.new("duncan", size: 41)
		end.to raise_exception(ArgumentError)
	end
	
	it "returns the error correction level" do
		# attr_reader was wrong
		expect(QRCode::Code.new("a", level: :h).error_correction_level).to be == :h
	end
	
	it "returns the version table" do
		# tables in QRCode::Code::QRMAXBITS wasn't updated to support greater versions
		expect(QRCode::Code.new("1" * 289, level: :h, mode: :number).version).to be == 11
		expect(QRCode::Code.new("A" * 175, level: :h, mode: :alphanumeric).version).to be == 11
		expect(QRCode::Code.new("a" * 383, level: :h, mode: :byte_8bit).version).to be == 21
	end
	
	it "returns the levels" do
		QRCode::Code.new("duncan", level: :l)
		QRCode::Code.new("duncan", level: :m)
		QRCode::Code.new("duncan", level: :q)
		QRCode::Code.new("duncan", level: :h)
		
		%w[a b c d e f g i j k n o p r s t u v w x y z].each do |ltr|
			expect do
				QRCode::Code.new("duncan", level: ltr.to_sym)
			end.to raise_exception(ArgumentError)
		end
	end
	
	it "supports UTF-8" do
		qr = QRCode::Code.new("тест")
		expect(qr.modules).to be == MATRIX_UTF8_RU_TEST
	end
	
	it "supports large integers" do
		QRCode::Code.new((1 << 64).to_s, mode: :number)
		QRCode::Code.new(((1 << 64) + 1).to_s, mode: :number)
	end
	
	it "handles code length overflows" do
		QRCode::Code.new("s" * 220)
		QRCode::Code.new("s" * 195)
	end
end
